...
{$ENDIF}
 ...
 TRpPrinterSelect=(pRpDefaultPrinter,pRpReportPrinter,
  ...
  pRpPrinter1,pRpPrinter2,pRpPrinter3,pRpPrinter4,pRpPrinter5,pRpPrinter6,pRpPrinter7,pRpPrinter8,pRpPrinter9,pRpPrinter10,
  pRpPrinter11, pRpPrinter12, pRpPrinter13, pRpPrinter14, pRpPrinter15, pRpPrinter16, pRpPrinter17, pRpPrinter18, pRpPrinter19, pRpPrinter20,
  pRpPrinter21, pRpPrinter22, pRpPrinter23, pRpPrinter24, pRpPrinter25, pRpPrinter26, pRpPrinter27, pRpPrinter28, pRpPrinter29, pRpPrinter30,
  pRpPrinter31, pRpPrinter32, pRpPrinter33, pRpPrinter34, pRpPrinter35, pRpPrinter36, pRpPrinter37, pRpPrinter38, pRpPrinter39, pRpPrinter40,
  pRpPrinter41, pRpPrinter42, pRpPrinter43, pRpPrinter44, pRpPrinter45, pRpPrinter46, pRpPrinter47, pRpPrinter48, pRpPrinter49, pRpPrinter50
  );
...
{$IFDEF DELPHI2009UP}
       displayformat:=FormatSettings.ShortTimeFormat
{$ELSE}
       displayformat:=ShortTimeFormat
{$ENDIF}
      else
{$IFDEF DELPHI2009UP}
       displayformat:=FormatSettings.ShortDateFormat+' '+FormatSettings.ShortTimeFormat;
{$ELSE}
...
begin
 ...
 begin
  ...
  begin
   if not FileExists(userconfigfilename) then
   begin
    configfilename:=Obtainininameuserconfig('','','reportman');
   end
   else
...
FNumero:=Abs(Fnumero);
{$IFDEF DELPHI2009UP}
fseparador:=FormatSettings.decimalseparator;
{$ELSE}
...
procedure ExecuteSystemApp(aparams:TStrings;waitend:Boolean);
begin
 Raise Exception.Create('ExecuteSystemApp not implemented');
end;
...
// apath:=AllocMem(alen+1);
 try
  if 0=GetTempPath(MAX_PATH,apath) then
   RaiseLastOsError;
...
procedure SendMail(destination,subject,content:AnsiString;files,orignalfilenames:TStrings);overload;
type
  TMapiFileDescs = array[0..50] of TMapiFileDesc;
...
begin
 ...
 try
  ...
  begin
   ...
   begin
    amessage.nFileCount:=files.Count;
    GetMem(PtrMapiFileDescs,  files.Count *
       SizeOf(MapiFileDesc));
       // zero this all out
       FillChar(PtrMapiFileDescs^, files.Count *
       SizeOf(MapiFileDesc), 0);
    amessage.lpFiles:= @PtrMapiFileDescs^;
...
{$IFDEF DELPHI2009UP}
 decchar:=FormatSettings.decimalseparator;
{$ELSE}
...
{$ENDIF}
 ...
 if decchar=chr(0) then
  decchar:='.';
{$IFDEF DELPHI2009UP}
 thchar:=FormatSettings.thousandseparator;
{$ELSE}
...
{$IFDEF DELPHI2009UP}
 index:=Pos(FormatSettings.decimalseparator,astring);
{$ELSE}
