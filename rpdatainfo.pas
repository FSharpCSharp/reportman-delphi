...
{$IFDEF DELPHI2009UP}
  DBXInterbase, DBXMySQL,DbxSqlite,
...
type
  TRpDbDriver = (rpdatadbexpress = 0, rpdatamybase = 1, rpdataibx = 2,
    rpdatabde = 3, rpdataado = 4, rpdataibo = 5, rpdatazeos = 6,
    rpdatadriver = 7, rpdotnet2driver = 8, rpfiredac = 9);
  ...
  public
    ...
    procedure GetDriverLibNames(const drivername: string;
      var LibraryName, VendorLib: string);
  ...
  IRpDatabaseDriver = interface
    ...
    function GetStreamFromSQL(sqlsentence: String; params: TStringList;
      paramlist: TRpParamList): TStream;
    procedure GetTableNames(alist: TStrings; params: TRpParamList);
    procedure GetFieldNames(atable: String;
      fieldlist, fieldtypes, fieldsizes: TStrings; params: TRpParamList);
    function OpenDatasetFromSQL(sqlsentence: String; params: TStringList;
      onlyexec: Boolean; paramlist: TRpParamList): TDataset;
  end;
...
{$ENDIF}
    ...
    function OpenDatasetFromSQL(sqlsentence: String; params: TStringList;
      onlyexec: Boolean; paramlist: TRpParamList): TDataset;
    procedure CreateLibrary(reporttable, reportfield, reportsearchfield,
      groupstable: String; paramlist: TRpParamList);
    function GetReportStream(ReportName: widestring;
      paramlist: TRpParamList): TStream;
    procedure SaveReportStream(ReportName: widestring; astream: TStream;
      paramlist: TRpParamList);
{$IFDEF USEADO}
...
{$ENDIF}
    ...
    property ADOConnectionString: widestring read FADOConnectionString
      write FADOConnectionString;
    procedure DoCommit;
  published
    property Alias: string read FAlias write SetAlias;
    property ConfigFile: string read FConfigFile write SetConfigFile;
    property LoadParams: Boolean read FLoadParams write SetLoadParams;
    property LoadDriverParams: Boolean read FLoadDriverParams
      write SetLoadDriverParams;
    property LoginPrompt: Boolean read FLoginPrompt write SetLoginPrompt;
    property Driver: TRpDbDriver read FDriver write FDriver
      default rpdatadbexpress;
    property reporttable: String read FReportTable write FReportTable;
    property reportsearchfield: String read FReportSearchField
      write FReportSearchField;
    property reportfield: String read FReportField write FReportField;
    property ReportGroupsTable: String read FReportGroupsTable
      write FReportGroupsTable;
  end;
...
{$ENDIF}
    ...
    function GetReportStream(ConnectionName: String; ReportName: widestring;
      paramlist: TRpParamList): TStream;
    procedure SaveReportStream(ConnectionName: String; ReportName: widestring;
      astream: TStream; paramlist: TRpParamList);
    ...
    property Items[index: integer]: TRpDatabaseInfoItem read GetItem
...
{$ENDIF}
  public
    ...
    property OnDisConnect: TDatasetNotifyEvent read FOnDisConnect
      write FOnDisConnect;
    procedure Assign(Source: TPersistent); override;
...
{$IFDEF USEADO}
    property externalDataset: Pointer read FexternalDataSet
      write FexternalDataSet;
{$ENDIF}
  published
    property Alias: string read FAlias write SetAlias;
    ...
    property MyBaseIndexFields: string read FMyBaseIndexFields
      write FMyBaseIndexFields;
    property MyBaseMasterFields: string read FMyBaseMasterFields
      write FMyBaseMasterFields;
    ...
    property BDEMasterFields: string read FBDEMasterFields
      write FBDEMasterFields;
    ...
    property GroupUnion: Boolean read FGroupUnion write FGroupUnion
      default false;
    property OpenOnStart: Boolean read FOpenOnStart write FOpenOnStart
      default true;
    property ParallelUnion: Boolean read FParallelUnion write FParallelUnion
      default false;
  end;
  ...
  public
    ...
    property Items[index: integer]: TRpDataInfoItem read GetItem
      write SetItem; default;
...
{$IFDEF USERPDATASET}
{$IFDEF FPC}
procedure CombineAddDataset(Client: TMemDataset; Data: TDataset;
  group: Boolean);
function CombineParallel(data1: TMemDataset; data2: TDataset; prefix: string;
  commonfields: TStrings; originalfields: TStrings): TMemDataset;
{$ENDIF}
{$IFNDEF FPC}
procedure CombineAddDataset(Client: TClientDataset; Data: TDataset;
  group: Boolean);
function CombineParallel(data1: TClientDataset; data2: TDataset; prefix: string;
  commonfields: TStrings; originalfields: TStrings): TClientDataset;
{$IFDEF USEIBX}
...
procedure FillFieldsInfo(adata: TDataset;
  fieldnames, fieldtypes, fieldsizes: TStrings);
...
begin
  ...
  if index >= 0 then
  begin
    drivername:= params.Values['DriverName'];
    if ((UpperCase(drivername) <> 'INTERBASE') AND (UpperCase(drivername) <> 'FIREBIRD')) then
...
begin
  ...
  index := params.IndexOfName('DriverID');
  if index < 0 then
    Raise Exception.Create(SRpNoDriverName);
  ...
  if index < 0 then
  begin
    index := params.IndexOfName('DataSource');
    if (index<0) then
      Raise Exception.Create(SRpNoDatabase);
  end;
  // params.Delete(index);
...
{$IFDEF FIREDAC}
  ...
  begin
    FFDInternalConnection.free;
  end;
  if Assigned(FFDInternalTransaction) then
  begin
...
procedure TRpDatabaseInfoList.SetItem(Index: integer;
  Value: TRpDatabaseInfoItem);
begin
  inherited SetItem(Index, Value);
end;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        begin
          paramlist.Add(Copy(paramname, 10 + Length(Alias),
            Length(paramname) - 1) + '=' + params.Items[i].AsString);
        end;
      end;
    end;
    case FDriver of
...
{$IFDEF USESQLEXPRESS}
          ...
          begin
            ...
            funcname := ConAdmin.drivers.ReadString(drivername,
              'GetDriverFunc', '');
...
{$IFDEF USEIBX}
          ...
          begin
            ...
            FIBInternalDatabase.DefaultTransaction :=
              TIBTransaction.Create(nil);
            FIBInternalDatabase.DefaultTransaction.DefaultDatabase :=
              FIBInternalDatabase;
...
{$IFDEF USEBDE}
          ...
          begin
            ...
            if Assigned(TRpDatabaseInfoList(Collection).FBDESession) then
              FBDEDatabase.SessionName := TRpDatabaseInfoList(Collection)
                .FBDESession.SessionName;
          end;
          ...
          begin
            if ((UpperCase(ASession.Databases[i].DatabaseName)
              = UpperCase(FBDEAlias)) or
              (UpperCase(ASession.Databases[i].AliasName)
              = UpperCase(FBDEAlias))) then
            begin
...
{$IFDEF USEADO}
          ...
          begin
            ...
            if (TRpDatabaseInfoList(Collection).FReport is TRpBasereport) then
            begin
              Report := TRpDatabaseInfoList(Collection)
                .FReport As TRpBasereport;
              if Report.params.IndexOf(Alias + '_ADOCONNECTIONSTRING') >= 0 then
                ADOConnection.ConnectionString :=
                  Report.params.ParamByName
                  (Alias + '_ADOCONNECTIONSTRING').AsString
              else if Report.params.IndexOf('ADOCONNECTIONSTRING') >= 0 then
                ADOConnection.ConnectionString :=
                  Report.params.ParamByName('ADOCONNECTIONSTRING').AsString;
            end;
...
{$IFDEF FIREDAC}
          ...
          if Not Assigned(FFDTransaction) then
          begin
            FFDInternalTransaction := TFDTransaction.Create(nil);
            FFDInternalTransaction.Connection := FFDConnection;
            FFDTransaction := FFDInternalTransaction;
          end;
          ...
          FFDConnection.LoginPrompt := FLoginPrompt;
          conname := Alias;
          // Load Connection parameters
          if (FLoadParams) then
          begin
            if Not Assigned(ConAdmin) then
              UpdateConAdmin;
            ConAdmin.GetConnectionParams(conname, FFDConnection.params);
          end;
...
{$IFDEF FIREDAC}
  if Assigned(FFDInternalTransaction) then
    if FFDInternalTransaction.Active then
      FFDInternalTransaction.Commit;
  if Assigned(FFDInternalConnection) then
  begin
...
procedure TRpDataInfoItem.Connect(databaseinfo: TRpDatabaseInfoList;
  params: TRpParamList);
var
  Index: integer;
  i: integer;
...
{$ENDIF}
    ...
    begin
      ...
      begin
        ...
        if param.ParamType in [rpParamSubst, rpParamSubstE, rpParamSubstList,
          rpParamMultiple] then
        begin
          index := param.Datasets.IndexOf(Alias);
          if index >= 0 then
          begin
            if ((param.ParamType = rpParamSubstE) or
              (param.ParamType = rpParamSubstList)) then
              sqlsentence := StringReplace(sqlsentence, param.Search,
                param.LastValue, [rfReplaceAll, rfIgnoreCase])
            else
              sqlsentence := StringReplace(sqlsentence, param.Search,
                param.Value, [rfReplaceAll, rfIgnoreCase]);
          end;
        end;
      end;
      ...
      begin
        ...
        if index < 0 then
          Raise Exception.Create(SRPMasterNotFound + Alias + ' - ' +
            DataSource);
...
{$IFDEF USEBDE}
              ...
              begin
                ...
                if Assigned(databaseinfo.FBDESession) then
                  TQuery(FSQLInternalQuery).SessionName :=
                    databaseinfo.FBDESession.SessionName;
              end
              else
              begin
                FSQLInternalQuery := TTable.Create(nil);
                if Assigned(databaseinfo.FBDESession) then
                  TTable(FSQLInternalQuery).SessionName :=
                    databaseinfo.FBDESession.SessionName;
              end;
{$ELSE}
              Raise Exception.Create(SRpDriverNotSupported + ' - ' +
...
{$IFDEF FIREDAC}
              ...
              TFDQuery(FSQLInternalQuery).FetchOptions.CursorKind :=
                ckForwardOnly;
{$ELSE}
...
{$ENDIF}
        ...
        rpdataibx:
          begin
{$IFDEF USEIBX}
            TIBQuery(FSQLInternalQuery).Database := baseinfo.FIBDatabase;
...
{$IFNDEF FPC}
                  ...
                  FillClientDatasetFromFile(TClientDataset(FSQLInternalQuery),
                    baseinfo.FMyBasePath + FMyBaseFields, afilename,
                    FMyBaseIndexFields);
{$ENDIF}
{$IFDEF FPC}
                  FillClientDatasetFromFile(TMemDataset(FSQLInternalQuery),
                    baseinfo.FMyBasePath + FMyBaseFields, afilename,
                    FMyBaseIndexFields);
{$ENDIF}
                end
...
{$IFNDEF FPC}
        ...
        begin
          TClientDataset(FSQLInternalQuery).IndexDefs.Clear;
          TClientDataset(FSQLInternalQuery).FieldDefs.Clear;
          TClientDataset(FSQLInternalQuery).IndexDefs.Add('IPRIM',
            FMyBaseIndexFields, []);
          TClientDataset(FSQLInternalQuery).IndexFieldNames :=
            FMyBaseIndexFields;
        end;
{$ENDIF}
        ...
        try
          ...
          begin
            ...
            if index < 0 then
              Raise Exception.Create(SRpDataUnionNotFound + ' - ' + Alias +
                ' - ' + FDataUnions.Strings[i]);
            TRpDataInfoList(Collection).Items[index]
              .Connect(databaseinfo, params);
            if ((i = 0) or (not FParallelUnion)) then
            begin
{$IFNDEF FPC}
              CombineAddDataset(TClientDataset(FSQLInternalQuery),
                TRpDataInfoList(Collection).Items[index].Dataset, FGroupUnion);
{$ENDIF}
{$IFDEF FPC}
              CombineAddDataset(TMemDataset(FSQLInternalQuery),
                TRpDataInfoList(Collection).Items[index].Dataset, FGroupUnion);
{$ENDIF}
...
{$IFNDEF FPC}
              ndataset := CombineParallel(TClientDataset(FSQLInternalQuery),
                TRpDataInfoList(Collection).Items[index].Dataset,
                'Q' + FormatFloat('00', i + 1) + '_', commonfields,
                originalfields);
              try
                FSQLInternalQuery.Close;
                TClientDataset(FSQLInternalQuery).FieldDefs.Clear;
                CombineAddDataset(TClientDataset(FSQLInternalQuery), ndataset,
                  FGroupUnion);
              finally
                ndataset.free;
              end;
...
{$IFDEF FPC}
              ...
              try
                ...
                CombineAddDataset(TMemDataset(FSQLInternalQuery), ndataset,
                  FGroupUnion);
              finally
                ndataset.free;
              end;
...
{$IFDEF FPC}
              ...
              begin
                raise Exception.Create('Filter not implemented in memdataset');
              end;
{$ENDIF}
            except
              on E: Exception do
...
{$IFDEF USEBDE}
        if FBDEType = rpdquery then
        begin
          TQuery(FSQLInternalQuery).DatabaseName :=
            baseinfo.FBDEDatabase.DatabaseName;
        end
        else
        begin
          TTable(FSQLInternalQuery).DatabaseName :=
            baseinfo.FBDEDatabase.DatabaseName;
        end;
...
{$IFDEF USEADO}
        ...
        begin
          TADOQuery(FSQLInternalQuery).Recordset :=
            _Recordset(FexternalDataSet);
...
{$ENDIF}
    ...
    rpfiredac:
      begin
{$IFDEF FIREDAC}
        TFDQuery(FSQLInternalQuery).Connection := baseinfo.FFDConnection;
      ...
      begin
        case baseinfo.Driver of
          rpdatadbexpress:
            begin
{$IFDEF USESQLEXPRESS}
              TSQLQuery(FSQLInternalQuery).ParamByName(param.Name)
                .DataType := atype;
              TSQLQuery(FSQLInternalQuery).ParamByName(param.Name).Value
                := avalue;
{$ENDIF}
            end;
          rpdataibx:
            begin
{$IFDEF USEIBX}
              TIBQuery(FSQLInternalQuery).ParamByName(param.Name)
                .DataType := atype;
              TIBQuery(FSQLInternalQuery).ParamByName(param.Name).Value
                := avalue;
{$ENDIF}
            end;
          rpdatazeos:
            begin
{$IFDEF USEZEOS}
              TZReadOnlyQuery(FSQLInternalQuery).ParamByName(param.Name)
                .DataType := atype;
              TZReadOnlyQuery(FSQLInternalQuery).ParamByName(param.Name).Value
                := avalue;
{$ENDIF}
            end;
          rpdatabde:
...
{$IFDEF USEBDE}
              if FBDEType = rpdquery then
              begin
                TQuery(FSQLInternalQuery).ParamByName(param.Name)
                  .DataType := atype;
                TQuery(FSQLInternalQuery).ParamByName(param.Name).Value
                  := avalue;
              end
              else
...
{$IFDEF USEIBO}
              ...
              TIBOQuery(FSQLInternalQuery).ParamByName(param.Name).Value
                := avalue;
{$ENDIF}
            end;
          rpfiredac:
...
{$IFDEF FIREDAC}
              TFDQuery(FSQLInternalQuery).Connection := baseinfo.FFDConnection;
              TFDQuery(FSQLInternalQuery).SQL.Text := sqlsentence;
              TFDQuery(FSQLInternalQuery).Transaction :=
                baseinfo.FFDTransaction;
              // TFDQuery(FSQLInternalQuery).Unidirectional:=true;
              TFDQuery(FSQLInternalQuery).DataSource := nil;
{$ENDIF}
            end;
...
{$ENDIF}
            ...
            AssignParamValuesS(TSQLQuery(FSQLInternalQuery),
              datainfosource.Dataset);
...
{$ENDIF}
              FMasterSource.Dataset := datainfosource.Dataset;
            AssignParamValuesIBX(TIBQuery(FSQLInternalQuery),
              datainfosource.Dataset);
{$ENDIF}
          end;
        rpdatazeos:
...
{$ENDIF}
              FMasterSource.Dataset := datainfosource.Dataset;
            AssignParamValuesZ(TZReadOnlyQuery(FSQLInternalQuery),
              datainfosource.Dataset);
{$ENDIF}
          end;
        rpdatamybase:
          begin
{$IFNDEF FPC}
            TClientDataset(FSQLInternalQuery).MasterFields :=
              MyBaseMasterFields;
...
{$ENDIF}
              FMasterSource.Dataset := datainfosource.Dataset;
            AssignParamValuesFiredac(TFDQuery(FSQLInternalQuery),
              datainfosource.Dataset);
{$ENDIF}
          end;
      end;
...
{$ENDIF}
  ...
  begin
    connecting := false;
    (* {$IFDEF USEADO}
      // OleAuto can not be included into a package
    ...
    begin
      if UpperCase(alist.Names[i]) <> 'GETDRIVERFUNC' then
        if UpperCase(alist.Names[i]) <> 'VENDORLIB' then
          ...
          begin
            config.WriteString(newname, alist.Names[i],
              alist.Values[alist.Names[i]]);
          end;
    end;
  finally
    alist.free;
...
begin
  Result := GetRegistryFile(SCONNECTIONREG_SETTING, SConnectionConfigFile,
    DesignMode);
end;
{$ENDIF}
...
{$IFDEF LINUX}
    ...
    begin
      ...
      begin
        if configdir <> '/usr/local/etc' then
        begin
          CopyFileTo('/usr/local/etc/' + DBXDRIVERFILENAME + '.conf',
            driverfilename);
        end;
        drivers := TMemInifile.Create(driverfilename);
      end
      else
        Raise Exception.Create(SRpConfigFileNotExists + ' - ' +
          DBXDRIVERFILENAME);
    end;
{$ENDIF}
  end;
...
{$ENDIF}
  if (not FileExists(configfilename)) then
  begin
    configfilename := Obtainininamelocaluserconfig('', 'Programs',
      'dbxconnections')
  end;
...
{$IFDEF MSWINDOWS}
    // Get public folder
    nconfigfilename := GetPublicPathSlash;
    if (Length(nconfigfilename) = 0) then
      Raise Exception.Create(SRpConfigFileNotExists + ' - ' + configfilename);
    configfilename := nconfigfilename + 'dbxconnections.ini';
    config := TMemInifile.Create(configfilename);
{$IFDEF USEVARIANTS}
{$IFNDEF FPC}
    config.CaseSensitive := false;
...
{$ENDIF}
    ...
    begin
      ...
      begin
        CopyFileTo('/usr/local/etc/' + DBXCONFIGFILENAME + '.conf',
          configfilename);
        config := TMemInifile.Create(configfilename);
{$IFNDEF FPC}
        config.CaseSensitive := false;
...
procedure TRpConnAdmin.GetDriverLibNames(const drivername: string;
  var LibraryName, VendorLib: string);
begin
...
function TRpDatabaseInfoItem.GetStreamFromSQL(sqlsentence: String;
  params: TStringList; paramlist: TRpParamList): TStream;
var
  Data: TDataset;
  memstream: TMemoryStream;
  astream: TStream;
...
procedure TRpDataInfoItem.GetFieldNames(fieldlist, fieldtypes,
  fieldsizes: TStrings);
var
  Report: TRpReport;
  alist: TStringList;
...
{$IFDEF MSWINDOWS}
          ...
          begin
            ...
            dwXCountChars := 80;
            dwYCountChars := 25;
            dwFillAttribute := FOREGROUND_RED or BACKGROUND_RED or
          ...
          FCommandLine := ' -deletereport -showfields ' + Alias + ' "' + tmpfile
            + '" "' + astring + '"';
          if Not CreateProcess(PChar(FExename), PChar(FCommandLine), nil, nil,
            true, NORMAL_PRIORITY_CLASS or CREATE_NEW_PROCESS_GROUP, nil, nil,
...
procedure TRpDatabaseInfoItem.GetFieldNames(atable: String;
  fieldlist, fieldtypes, fieldsizes: TStrings; params: TRpParamList);
{$IFDEF USEZEOS}
var
  res: IZResultSet;
...
{$ENDIF}
    ...
    rpdatazeos:
      begin
{$IFDEF USEZEOS}
        res := FZConnection.DbcConnection.GetMetadata.GetColumns('', '',
          atable, '');
        fieldlist.Clear;
        if res.First then
...
{$IFDEF USEBDE}
        ...
        try
          ...
          if Assigned(TRpDatabaseInfoList(Collection).FBDESession) then
            BDETable.SessionName := TRpDatabaseInfoList(Collection)
              .FBDESession.SessionName;
...
{$ENDIF}
    ...
    rpdatazeos:
      begin
{$IFDEF USEZEOS}
        res := FZConnection.DbcConnection.GetMetadata.GetTables('', '',
          '', nil);
        alist.Clear;
        if res.First then
...
{$IFDEF USEBDE}
        if Assigned(TRpDatabaseInfoList(Collection).FBDESession) then
          TRpDatabaseInfoList(Collection).FBDESession.GetTableNames
            (FBDEDatabase.DatabaseName, '', false, false, alist)
        else
          Session.GetTableNames(FBDEDatabase.DatabaseName, '', false,
            false, alist);
        // GetTableNames(alist);
{$ELSE}
        Raise Exception.Create(SRpDriverNotSupported + ' - ' + SrpDriverBDE);
...
function TRpDatabaseInfoItem.OpenDatasetFromSQL(sqlsentence: String;
  params: TStringList; onlyexec: Boolean; paramlist: TRpParamList): TDataset;
var
  FSQLInternalQuery: TDataset;
  i: integer;
  astream: TStream;
...
{$IFDEF USEBDE}
        ...
        if Assigned(TRpDatabaseInfoList(Collection).FBDESession) then
          TQuery(FSQLInternalQuery).SessionName :=
            TRpDatabaseInfoList(Collection).FBDESession.SessionName;
...
{$IFDEF FIREDAC}
        FSQLInternalQuery := TFDQuery.Create(nil);
        TFDQuery(FSQLInternalQuery).Connection := FFDConnection;
        TFDQuery(FSQLInternalQuery).SQL.Text := sqlsentence;
{$ELSE}
        Raise Exception.Create(SRpDriverNotSupported + 'FireDAC');
{$ENDIF}
...
{$IFDEF USESQLEXPRESS}
              ...
              begin
                TSQLQuery(FSQLInternalQuery).ParamByName(paramname).DataType
                  := ftBlob;
                TSQLQuery(FSQLInternalQuery).ParamByName(paramname)
              ...
              begin
                ...
                TSQLQuery(FSQLInternalQuery).ParamByName(paramname).Value
                  := avariant;
              end;
{$ENDIF}
            end;
...
{$IFDEF USEIBX}
              ...
              begin
                TIBQuery(FSQLInternalQuery).ParamByName(paramname).DataType
                  := ftBlob;
                TIBQuery(FSQLInternalQuery).ParamByName(paramname)
                  .LoadFromStream(astream, ftBlob);
              end
              else
              begin
                TIBQuery(FSQLInternalQuery).ParamByName(paramname).DataType :=
                  VariantTypeToDataType(avariant);
                TIBQuery(FSQLInternalQuery).ParamByName(paramname).Value
                  := avariant;
              end;
{$ENDIF}
            end;
...
{$IFDEF USEZEOS}
              if Assigned(astream) then
              begin
                TZReadOnlyQuery(FSQLInternalQuery).ParamByName(paramname)
                  .DataType := ftBlob;
                TZReadOnlyQuery(FSQLInternalQuery).ParamByName(paramname)
                  .LoadFromStream(astream, ftBlob);
              end
              ...
              begin
                TZReadOnlyQuery(FSQLInternalQuery).ParamByName(paramname)
                  .DataType := VariantTypeToDataType(avariant);
                TZReadOnlyQuery(FSQLInternalQuery).ParamByName(paramname).Value
                  := avariant;
              end;
{$ENDIF}
            end;
...
{$IFDEF USEBDE}
              ...
              begin
                TQuery(FSQLInternalQuery).ParamByName(paramname).DataType
                  := ftBlob;
                TQuery(FSQLInternalQuery).ParamByName(paramname)
                  .LoadFromStream(astream, ftBlob);
              end
              else
              begin
                TQuery(FSQLInternalQuery).ParamByName(paramname).DataType :=
                  VariantTypeToDataType(avariant);
                TQuery(FSQLInternalQuery).ParamByName(paramname).Value
                  := avariant;
              end;
{$ENDIF}
            end;
...
{$IFDEF USEADO}
              if Assigned(astream) then
              begin
                for j := 0 to TADOQuery(FSQLInternalQuery)
...
{$IFDEF USEIBO}
              if Assigned(astream) then
              begin
                TIBOQuery(FSQLInternalQuery).ParamByName(paramname)
                  .Assign(astream);
              end
              else
              begin
                // TIBOQuery(FSQLInternalQuery).ParamByName(paramName).AsVariant:=avariant;
                TIBOQuery(FSQLInternalQuery).ParamByName(paramname).Value
                  := avariant;
              end;
{$ENDIF}
            end;
...
{$IFDEF FIREDAC}
              ...
              begin
                TFDQuery(FSQLInternalQuery).ParamByName(paramname).DataType
                  := ftBlob;
                TFDQuery(FSQLInternalQuery).ParamByName(paramname)
                  .LoadFromStream(astream, ftBlob);
              end
              else
              begin
                TFDQuery(FSQLInternalQuery).ParamByName(paramname).Value
                  := avariant;
              end;
{$ENDIF}
            end;
...
{$IFNDEF FPC}
  ...
  function CombineParallel(data1: TClientDataset; data2: TDataset;
    prefix: string; commonfields: TStrings; originalfields: TStrings)
    : TClientDataset;
  var
    ...
    try
      ...
      begin
        ...
        begin
          ...
          begin
            aresult.FieldByName(originalfields[i]).AsVariant :=
              data2.FieldByName(commonfields.Strings[i]).AsVariant;
          end;
          aresult.FieldByName(prefix).Value := 0;
        ...
        try
          ...
          begin
            aresult.FieldByName(lfields2.Strings[i]).AsVariant :=
              data2.FieldByName(lfields1.Strings[i]).AsVariant;
          end;
...
{$IFNDEF FPC}
          ...
          begin
            groupfieldindex.Add
              (IntToStr(Client.FieldByName(groupfields.Strings[i]).Index));
          end;
{$ENDIF}
{$IFDEF FPC}
                  ...
                  begin
                    ...
                    begin
                      if Client.fields[i].IsNull then
                        ...
                        Client.fields[i].AsVariant := Client.fields[i].AsVariant
                          + Data.fields[i].AsVariant;
                    end;
                  end;
                end;
                Client.Post;
...
{$ENDIF}
    ...
    begin
      ...
      Filer.DefineProperty('ADOConnectionString', ReadAdoConnectionString,
        WriteAdoConnectionString, true);
    end;
    ...
    begin
      ...
      try
        ...
        begin
          ...
          inif.WriteString(conname, 'REPORTSEARCHFIELD',
            aitem.reportsearchfield);
          inif.WriteString(conname, 'REPORTGROUPSTABLE',
            aitem.ReportGroupsTable);
        end;
        inif.UpdateFile;
      finally
        inif.free;
    ...
    begin
      ...
      try
        ...
        begin
          ...
          aitem := Add(AName);
          aitem.ADOConnectionString := inif.ReadString(conname, 'ADOSTRING',
            aitem.ADOConnectionString);
          aitem.LoadParams := inif.ReadBool(conname, 'LOADPARAMS',
            aitem.LoadParams);
          aitem.LoadDriverParams := inif.ReadBool(conname, 'LOADDRIVERPARAMS',
            aitem.LoadDriverParams);
          aitem.LoginPrompt := inif.ReadBool(conname, 'LOGINPROMPT',
            aitem.LoginPrompt);
          aitem.Driver := TRpDbDriver(inif.ReadInteger(conname, 'DRIVER',
            integer(aitem.Driver)));
          aitem.reporttable := inif.ReadString(conname, 'REPORTTABLE',
            aitem.reporttable);
          aitem.reportfield := inif.ReadString(conname, 'REPORTFIELD',
            aitem.reportfield);
          aitem.reportsearchfield := inif.ReadString(conname,
            'REPORTSEARCHFIELD', aitem.reportsearchfield);
          aitem.ReportGroupsTable := inif.ReadString(conname,
            'REPORTGROUPSTABLE', aitem.ReportGroupsTable);
        end;
      finally
        inif.free;
      end;
    ...
    procedure TRpDatabaseInfoItem.CreateLibrary(reporttable, reportfield,
      reportsearchfield, groupstable: String; paramlist: TRpParamList);
    var
      astring: String;
    begin
      // Creates the library
      astring := 'CREATE TABLE ' + reporttable + ' (' + reportsearchfield +
        ' VARCHAR(50) NOT NULL,' + reportfield +
        ' BLOB,REPORT_GROUP INTEGER,USER_FLAG INTEGER,PRIMARY KEY (' +
        reportsearchfield + '))';
    ...
    function TRpDatabaseInfoList.GetReportStream(ConnectionName: String;
      ReportName: widestring; paramlist: TRpParamList): TStream;
    var
      Index: integer;
    begin
    ...
    procedure TRpDatabaseInfoList.SaveReportStream(ConnectionName: String;
      ReportName: widestring; astream: TStream; paramlist: TRpParamList);
    var
      Index: integer;
    begin
      index := IndexOf(ConnectionName);
    ...
    function TRpDatabaseInfoItem.GetReportStream(ReportName: widestring;
      paramlist: TRpParamList): TStream;
    var
      astring: String;
      params: TStringList;
    ...
    begin
      Result := nil;
      astring := 'SELECT ' + reportfield + ' FROM ' + reporttable + ' WHERE ' +
        reportsearchfield + '=:REPNAME';
      params := TStringList.Create;
      try
        ...
        try
          ...
          try
            if adata.Eof and adata.Bof then
              Raise Exception.Create(SRptReportnotfound + ':' + Alias + ':' +
                ReportName);
    ...
    procedure TRpDatabaseInfoItem.SaveReportStream(ReportName: widestring;
      astream: TStream; paramlist: TRpParamList);
    var
      astring: String;
      params: TStringList;
    ...
    begin
      ...
      try
        ...
        try
          ...
          astring := 'SELECT ' + reportsearchfield + ' FROM ' + reporttable +
            ' WHERE ' + reportsearchfield + '=:REPNAME';
          adata := OpenDatasetFromSQL(astring, params, false, paramlist);
          try
            if adata.Eof and adata.Bof then
              Raise Exception.Create(SRptReportnotfound + ':' + Alias + ':' +
          ...
          astring := 'UPDATE ' + reporttable + ' SET ' + reportfield +
            '=:REPORT' + ' WHERE ' + reportsearchfield + '=:REPNAME';
...
{$IFDEF FPC}
    procedure TRpDatabaseInfoList.FillTreeDir(ConnectionName: String;
      alist: TStrings);
    begin
      Raise Exception.Create
        ('FIllTreeDir Not implemented on Freepascal version');
    end;
{$ENDIF}
{$IFNDEF FPC}
    procedure TRpDatabaseInfoList.FillTreeDir(ConnectionName: String;
    ...
    begin
      ...
      try
        ...
        begin
          if dbinfo.ReportGroupsTable = 'GINFORME' then
          begin
            adatagroups := dbinfo.OpenDatasetFromSQL
              ('SELECT CODIGO AS GROUP_CODE,NOMBRE AS GROUP_NAME,' +
              ' GRUPO AS PARENT_GROUP FROM ' + dbinfo.ReportGroupsTable, nil,
              false, nil);
          end
          else
            adatagroups := dbinfo.OpenDatasetFromSQL
              ('SELECT GROUP_CODE,GROUP_NAME,' + ' PARENT_GROUP FROM ' +
              dbinfo.ReportGroupsTable, nil, false, nil);
        end;
        try
          ...
          try
            ...
            begin
              ...
              begin
                ...
                try
                  DReportgroups.FieldByName('GROUP_CODE').Value :=
                    adatagroups.FieldByName('GROUP_CODE').Value;
                  DReportgroups.FieldByName('GROUP_NAME').AsVariant :=
                    adatagroups.FieldByName('GROUP_NAME').AsVariant;
                  DReportgroups.FieldByName('PARENT_GROUP').AsVariant :=
                    adatagroups.FieldByName('PARENT_GROUP').AsVariant;
                  DReportgroups2.Append;
                  try
                    for i := 0 to DReportgroups2.fields.Count - 1 do
                    begin
                      DReportgroups2.fields[i].AsVariant := DReportgroups.fields
                        [i].AsVariant;
                    end;
                    DReportgroups2.Post;
                  except
            ...
            begin
              Dreports.Append;
              try
                Dreports.FieldByName('REPORT_NAME').Value :=
                  adatareports.FieldByName('REPORT_NAME').Value;
                if havegroup then
                  Dreports.FieldByName('REPORT_GROUP').AsVariant :=
                    adatareports.FieldByName('REPORT_GROUP').AsVariant;
                Dreports.Post;
              except
                Dreports.Cancel;
                Raise;
            ...
            begin
              ...
              begin
                ...
                while ((Dreports.FieldByName('REPORT_GROUP')
                  .AsInteger = groupcode) and (Not Dreports.Eof)) do
                begin
                  alist.Add(grouppath + C_DIRSEPARATOR +
                    Dreports.FieldByName('REPORT_NAME').AsString);
                  Dreports.Next;
                end;
...
{$ENDIF}
    procedure FieldToDataString(afield: TField;
      var typestring, sizestring: String);
    begin
      sizestring := '';
...
{$IFDEF USEZEOS}
      if dtype = rpdatazeos then
        if Not EqualParamValuesZ(TZReadOnlyQuery(datainfoitem.Dataset),
          DataSource.Dataset) then
          reopen := true;
{$ENDIF}
{$IFDEF USESQLEXPRESS}
      ...
      begin
        ...
        begin
          if Not EqualParamValuesS(TSQLQuery(datainfoitem.Dataset),
            DataSource.Dataset) then
            reopen := true;
        end;
      end;
{$ENDIF}
{$IFDEF USEIBX}
      if dtype = rpdataibx then
      begin
        if datainfoitem.Dataset is TIBQuery then
        begin
          if Not EqualParamValuesIBX(TIBQuery(datainfoitem.Dataset),
            DataSource.Dataset) then
            reopen := true;
        end;
      end;
...
{$IFDEF USEZEOS}
        if dtype = rpdatazeos then
          AssignParamValuesZ(TZReadOnlyQuery(datainfoitem.Dataset),
            DataSource.Dataset);
{$ENDIF}
{$IFDEF USEIBX}
        if dtype = rpdataibx then
          AssignParamValuesIBX(TIBQuery(datainfoitem.Dataset),
            DataSource.Dataset);
{$ENDIF}
{$IFDEF FIREDAC}
        if dtype = rpfiredac then
          AssignParamValuesFiredac(TFDQuery(datainfoitem.Dataset),
            DataSource.Dataset);
{$ENDIF}
{$IFDEF USESQLEXPRESS}
        if dtype = rpdatadbexpress then
          AssignParamValuesS(TSQLQuery(datainfoitem.Dataset),
            DataSource.Dataset);
{$ENDIF}
