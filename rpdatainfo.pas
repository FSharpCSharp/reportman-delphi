 ...
 try
  ...
  {$IFDEF USEADO}
      ...
      begin
       ...
       begin
        report:=TRpDatabaseInfoList(Collection).FReport As TRpbASEReport;
...
{$ENDIF}
        FMasterSource.DataSet:=datainfosource.Dataset;
        AssignParamValuesIBX(TIBQuery(FSQLInternalQuery),datainfosource.Dataset);
{$ENDIF}
      end;
     rpdatazeos:
      begin
...
{$IFDEF USEIBX}
 ...
 begin
  ...
  begin
   if Not EqualParamValuesIBX(TIBQuery(datainfoitem.dataset),DataSource.Dataset) then
    reopen:=true;
  end;
 end;
{$ENDIF}
...
{$IFDEF USEIBX}
  if dtype=rpdataibx then
   AssignParamValuesIBX(TIBQuery(datainfoitem.dataset),DataSource.Dataset);
{$ENDIF}
{$IFDEF FIREDAC}
