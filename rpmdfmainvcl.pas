...
procedure TFRpMainFVCL.menuthemeClick(Sender: TObject);
var
...
begin
{$IFDEF DELPHI2009UP}
 for i:=0 to  menutheme.Count-1 do
   menutheme.Items[i].Checked := false;
 selecteditem:=TMenuItem(Sender);
 menuname:=StringReplace(selecteditem.Caption,'&','',[rfReplaceAll]);
 TStyleManager.SetStyle(menuname);
 ThemeName:=menuname;
...
begin
 ...
 try
  ...
  ThemeName:=inif.ReadString('Preferences','ThemeName','');
  if (Length(ThemeName)>0) then
  begin
   try
{$IFDEF DELPHI2009UP}
