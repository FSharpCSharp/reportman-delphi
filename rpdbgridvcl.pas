...
{$IFDEF DELPHI2009UP}
  if ((Key=VK_DECIMAL) AND (VK_DECIMAL<>Ord(FormatSettings.DecimalSeparator))) then
{$ELSE}
...
{$IFDEF DELPHI2009UP}
   PostMessage(handle,WM_CHAR,ord(FormatSettings.decimalseparator),0);
{$ELSE}
...
{$IFDEF DELPHI2009UP}
  if ((Key=chr(VK_DECIMAL)) AND (VK_DECIMAL<>Ord(FormatSettings.DecimalSeparator))) then
{$ELSE}
