...
//     Canvas.FillRect(rect);
     Canvas.Pixels[avalue.X,avalue.Y]:=Color;
     y:=y+XHeight;
    end;
    x:=x+XWidth;
   end;
...
begin
 ...
 begin
  if firstname=anode.Items[i].Text then
  begin
   ...
   begin
    ...
    Result:=Searchnodeint(ATree,Copy(astring,length(firstname)+2,length(astring)),anode.Items[i]);
  end;
 end;
