...
type
 TPDFReport=class(TCBaseReport)
  ...
  public
   ...
   procedure SaveToText(textfilename:string;textdriver:String='');override;
   procedure SaveToMetafile(metafilename:string);
  published
...
procedure TPDFReport.SaveToText(textfilename:string;textdriver:String='');
begin
 rptextdriver.PrintReportToText(report,'',false,true,1,999,
   1,textfilename,true,true,textdriver);
end;

procedure TPDFReport.SaveToMetafile(metafilename:string);
begin
 rppdfdriver.PrintReportToMetafile(Report,Title,ShowProgress,true,
  1,MAX_PAGECOUNT,1,metafilename,false);
end;
